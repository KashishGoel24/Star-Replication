...
class Server(Process):
  ...
  def handle_client(self, client_sock: socket.socket, addr: socket.AddressInfo, req_q: queue.Queue[Tuple[JsonMessage, socket.socket]]):
    ...
    try:
      while True:
        ...
        print("recvd the request from", addr)
        if request is None:
          ...
          print("putting the request into the queue")
  ...
  def run(self) -> None:
    ...
    cmd_thread = threading.Thread(target=self._process_req,args=(req_q,),
    ...
    try:
      while True:
        ...
        client_handler = threading.Thread(target=self.handle_client,
                                          args=(client_sock, addr, req_q), name=f"listen#{addr[1]}")
